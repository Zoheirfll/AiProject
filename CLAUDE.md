...
- `accounts` — authentication and authorization (added post-MVP, not in the original v3/v4 epics):
  ...
  - **Migration footgun**: `AUTH_USER_MODEL` was swapped on a DB that already had `admin`'s migrations applied (admin's `LogEntry.user` FK creates a dependency on the user model). Fixed once by deleting `django_migrations` rows for `app='admin'`, applying `accounts` first, then `migrate admin --fake` (tables were already correct, only the bookkeeping order was wrong). Not an issue going forward — only mattered for this one historical swap.
